...
演習5の実装に、新しい要求「顧客が予約をキャンセルする」を追加した。

本演習では、チェックイン前（予約済）の予約を、予約番号と氏名でキャンセルできる機能を実装した。
演習1〜5の成果物との差分（デルタ）として整理し、拡張が既存設計に与える影響を考察した。

## 演習6の成果物

- [保守・拡張説明](docs/exercise6_maintenance.md)
- [ユースケース図（差分） PlantUML](diagrams/exercise6_usecase_delta.puml)
- [予約キャンセル アクティビティ図 PlantUML](diagrams/exercise6_activity_cancel.puml)
- [予約キャンセル コラボレーション図 PlantUML](diagrams/exercise6_collaboration_cancel.puml)
- [演習6レビュー結果](reviews/exercise6_review.md)

## 演習6の工夫点

新機能を `CancelReservationControl` として独立させ、既存の予約・チェックイン・チェックアウト処理を変更せずに拡張した。

空室判定（`ReservationManager.availableRooms`）とチェックイン処理（`CheckInRoomControl`）にも、キャンセル済み予約を除外する変更を加え、状態遷移の整合性を保った。

ロバストネス分析、SOLID原則、3層アーキテクチャの観点から、拡張の影響を `docs/exercise6_maintenance.md` に記述した。
