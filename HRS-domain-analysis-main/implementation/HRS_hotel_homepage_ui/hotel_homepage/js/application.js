...
class ReservationManager {
  ...
  availableRooms(roomType, checkInDate, checkOutDate, guestCount, excludeReservationNo = "") {
    const activeReservations = this.list().filter(r => {
      if (r.status === "CHECKED_OUT" || r.status === "CANCELLED") return false;
  ...
  cancel(reservationNo, lastName, firstName) {
    const reservation = this.findByNo(reservationNo);
    if (!reservation) throw new Error("予約番号が見つかりません。");
    if (reservation.status === "CANCELLED") throw new Error("この予約はすでにキャンセル済みです。");
    if (reservation.status === "CHECKED_IN") throw new Error("チェックイン済みの予約はキャンセルできません。フロントへお問い合わせください。");
    if (reservation.status === "CHECKED_OUT") throw new Error("この予約はすでにチェックアウト済みです。");
    const ln = lastName.trim();
    const fn = firstName.trim();
    if (reservation.customer.lastName !== ln || reservation.customer.firstName !== fn) {
      throw new Error("予約者氏名が一致しません。姓・名をご確認ください。");
    }
...
class CheckInRoomControl {
  ...
  execute(reservationNo) {
    ...
    if (reservation.status === "CHECKED_OUT") throw new Error("この予約はすでにチェックアウト済みです。");
    if (reservation.status === "CANCELLED") throw new Error("この予約はキャンセル済みです。");
...
class CancelReservationControl {
  constructor(reservationManager) { this.reservationManager = reservationManager; }
  execute(input) {
    return this.reservationManager.cancel(input.reservationNo, input.lastName, input.firstName);
  }
}
